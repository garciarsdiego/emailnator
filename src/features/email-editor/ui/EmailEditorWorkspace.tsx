...
export function EmailEditorWorkspace(props: EmailEditorWorkspaceProps) {
  return (
    <div className="flex flex-1 overflow-hidden">
      {props.mode === "edit" && <>
        <aside className="w-44 overflow-y-auto border-r border-foreground/15 bg-card/70 p-2" aria-label="Blocos"><BlockPalette onAddBlock={props.onAddBlock} /></aside>
        <div className="flex-1 overflow-auto bg-[hsl(var(--graphite))] p-4">
        ...
        <aside className="w-64 overflow-y-auto border-l border-foreground/15 bg-card/90 p-3" aria-label="Propriedades do bloco">
      ...
      {props.mode === "preview" && (
        <div className="flex-1 overflow-auto bg-[hsl(var(--graphite))] p-4">
          <div className={cn("mx-auto transition-all", props.device === "mobile" ? "max-w-[375px]" : "max-w-[600px]")}>
            <Card className="overflow-hidden shadow-[var(--shadow-v3-graphite)]"><iframe srcDoc={props.html} sandbox="" className="min-h-[600px] w-full border-0" title="Preview do email" /></Card>
          </div>
        </div>
      )}

      {props.mode === "html" && <div className="flex-1 overflow-auto bg-[hsl(var(--graphite))] p-4"><Card className="p-4 shadow-[var(--shadow-v3-graphite)]"><pre className="max-h-[600px] overflow-auto rounded bg-muted p-4 text-sm"><code>{props.html}</code></pre></Card></div>}
    </div>
  );
}
